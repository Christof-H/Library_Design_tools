...
@pytest.fixture
def locus():
    primer_univ = ['BB297.Fw', 'GACTGGTACTCGCGTGACTTG', 'BB299.Rev', 'CCAGTCCAGAGGTGTCCCTAC']
    ...
    return locus, seq_list


def test_recover_genomic_seq_locus_length_strategy(locus):
    locus, seq_list = locus
    locus.design_type = 'locus_length'
    final_seq, start, end = locus.recover_genomic_seq(2, 3, 10000, seq_list)
    assert (len(final_seq) == 100 and start == 30000 and end == 50000)


def test_recover_genomic_seq_nbr_probes_strategy(locus):
    locus, seq_list = locus
